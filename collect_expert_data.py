...
def collect_expert_dataset(N: int, path: str, multi_task: bool):
    ...
    vel_all = torch.empty(max_steps, B, 2, dtype=torch.float32)
    u_all = torch.empty(max_steps, B, 2, dtype=torch.float32)
    obj_pos_all = torch.empty(max_steps, B, 2, dtype=torch.float32)
    expert_ctrl_pos_all = torch.empty(max_steps, B, 2, dtype=torch.float32)
    ...
    vel_arr = root.create_array('data/vel', shape=(0, 2), dtype=np.float32, chunks=(10000, 2))
    ...
    expert_ctrl_pos_arr = root.create_array('meta/expert_ctrl_pos', shape=(0, 2), dtype=np.float32, chunks=(10000, 2))
    ...
    for start in tqdm(range(0, int(N), B)):
        ...
        while bool(active.any()) and t < max_steps:
            ...
            frames_all[t] = env.current_frame().to("cpu")
            poss_all[t] = env.agent_pos.to("cpu")
            vel_all[t] = env.agent_vel.to("cpu")
            u_all[t] = u.to("cpu")
            for b in range(B):
                obj_idx = int(env._target_object_idx[b].item())
                obj_pos_all[t, b] = env.objects_pos[b, obj_idx].to(torch.float32).to("cpu")
        ...
        for b in range(B):
            t_end = int(done_step[b].item())
            if t_end < 0:
                print("INCOMPLETE DEMONSTRATION")
                t_end = max_steps
            ...
            vel_arr[total_samples:total_samples+t_end] = vel_all[:t_end, b].numpy()
            ...
            expert_ctrl_pos_arr[total_samples:total_samples+t_end] = expert_ctrl_pos_all[:t_end, b].numpy()
            total_samples += t_end
